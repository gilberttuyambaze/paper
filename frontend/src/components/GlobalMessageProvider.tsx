...
import { AlertCircle, AlertTriangle, CheckCircle2, HelpCircle, Info, LoaderCircle } from 'lucide-react';
...
export default function GlobalMessageProvider() {
  ...
  const close = () => { if (pendingAction === null) { setMessage(null); setActionError(null); } };
  ...
  return (
    <Dialog open={Boolean(message)} onOpenChange={(open) => !open && canDismiss && pendingAction === null && close()}>
      <DialogContent className={`w-[calc(100vw-2rem)] max-w-md overflow-hidden border p-0 shadow-2xl ${config.tone}`}>
        {message && <div className="bg-background/90 p-6 text-center backdrop-blur-sm sm:p-8">
          ...
          <DialogHeader className="items-center text-center">
            ...
            <DialogDescription className="theme-muted max-w-sm pt-2 text-sm leading-6">{actionError || message.message}</DialogDescription>
          </DialogHeader>
          <div className="mt-7 flex flex-wrap justify-center gap-3">
            {actions.map((action, index) => <Button key={`${action.label}-${index}`} variant={action.variant || (message.type === 'error' ? 'outline' : 'default')} disabled={pendingAction !== null} onClick={async () => {
              setPendingAction(index); setActionError(null);
              try {
                const result = await action.onClick();
                if (result !== false) { setMessage(null); setActionError(null); }
              } catch (error) {
                setActionError(error instanceof Error ? error.message : 'The request could not be completed.');
              } finally { setPendingAction(null); }
            }}>{pendingAction === index && <LoaderCircle className="mr-2 h-4 w-4 animate-spin" />}{pendingAction === index ? (action.loadingLabel || `${action.label}...`) : action.label}</Button>)}
          </div>
        </div>}
      </DialogContent>
    </Dialog>
