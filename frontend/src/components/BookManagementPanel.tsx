...
import { BookOpen, Clock, Eye, Lock, RotateCcw, Search, Trash2 } from 'lucide-react';
...
import { deleteBook, fetchBookActivity, fetchBooks, restoreBook, setBookStatus, updateBook, type Book, type BookStatus } from '@/lib/books';
...
export default function BookManagementPanel({ mode, userId }: Props) {
  ...
  const [confirmation, setConfirmation] = useState<{ book: Book; action: 'delete' | 'restore' } | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await fetchBooks(mode === 'admin' ? { include_deleted: true } : { uploaded_by: userId });
      setBooks(data.items);
    } catch (error) {
  ...
  const visible = useMemo(() => books.filter((book) => {
    ...
    return (!query || [book.title, book.authors.join(' '), book.uploader_name, book.language].filter(Boolean).some((value) => String(value).toLowerCase().includes(query)))
      && (status === 'all' || book.status === status || (status === 'deleted' && Boolean(book.deleted_at)))
  ...
  const remove = async (book: Book) => { try { await deleteBook(book.id); setBooks((items) => items.filter((item) => item.id !== book.id)); setSelectedBook(null); toast.success('Book deleted'); } catch (error) { operationError(error, 'Delete was denied'); } };
  const restore = async (book: Book) => { try { replace(await restoreBook(book.id)); toast.success('Book restored'); } catch (error) { operationError(error, 'Restore was denied'); } };
  ...
  const confirmAction = (book: Book, action: 'delete' | 'restore') => showMessage({ type: 'confirmation', title: action === 'delete' ? 'Delete Book?' : 'Restore Book?', message: action === 'delete' ? 'This removes the Book from public resources under the existing backend lifecycle policy.' : 'This restores the Book record.', actions: [{ label: 'Cancel', variant: 'outline', onClick: () => undefined }, { label: action === 'delete' ? 'Delete' : 'Restore', variant: action === 'delete' ? 'destructive' : 'default', onClick: () => action === 'delete' ? remove(book) : restore(book) }] });
