...
export default function AdminPage() {
  ...
  const loadData = async () => {
    try {
      ...
      books.forEach((book: any) => {
        if (book.status === 'active') stats.active++;
  ...
  const confirmDeletePaper = (paper: Paper) => showMessage({ type: 'confirmation', title: 'Delete Paper?', message: 'This permanently deletes the Paper and its stored files.', actions: [{ label: 'Cancel', variant: 'outline', onClick: () => undefined }, { label: 'Delete', loadingLabel: 'Deleting...', variant: 'destructive', onClick: async () => { try { await deleteMyPaper(paper.id); setPapers((items) => items.filter((item) => item.id !== paper.id)); setSelectedPaper(null); toast.success('Paper permanently deleted'); } catch (error) { toast.error(normalizeApiError(error).message || 'Unable to delete paper'); return false; } } }] });
