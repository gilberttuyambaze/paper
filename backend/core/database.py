...
class DatabaseManager:
        ...
        try:
            ...
            try:
                ...
                async with self.engine.begin() as conn:
                    ...
                    await self._ensure_model_columns(conn, "users")
                    await self._ensure_model_columns(conn, "user_profiles")
    ...
    async def _has_table_column(self, conn, table_name: str, column_name: str) -> bool:
        dialect_name = self.engine.dialect.name if self.engine else ""

        if dialect_name == "sqlite":
            result = await conn.execute(text(f"PRAGMA table_info('{table_name}')"))
            return any(row[1] == column_name for row in result.all())
        ...
        if dialect_name in ("mysql", "mariadb"):
            result = await conn.execute(
                text(
                    "SELECT 1 FROM information_schema.columns WHERE table_schema = database() AND table_name = :table_name AND column_name = :column_name"
                ),
                {"table_name": table_name, "column_name": column_name},
            )
            return result.first() is not None
    ...
    async def _ensure_model_columns(self, conn, table_name: str) -> None:
        if table_name not in Base.metadata.tables:
            return

        for column in self._get_model_columns(table_name):
            if await self._has_table_column(conn, table_name, column["name"]):
                continue
